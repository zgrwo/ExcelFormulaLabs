...
namespace ExcelVbaLibraries.Analytics
{
    public static class PhyChemUdf
    {
        ...
        [ExcelFunction(Name="PHYCHEM.VOL", Description="Convert volume between units: L, ML, M3, GAL, QT, FT3")] public static object UDF_PC_VOL(object v,object from,object to)=>OutputWrapper.WrapError(()=>ElementWiseMapper.MapOver<double,double>(v,d=>PhyChemCore.ConvertVolume(d,S(from),S(to))));
        [ExcelFunction(Name="PHYCHEM.MASS", Description="Convert mass between units: KG, G, MG, LB, OZ, TON")] public static object UDF_PC_MASS(object v,object from,object to)=>OutputWrapper.WrapError(()=>ElementWiseMapper.MapOver<double,double>(v,d=>PhyChemCore.ConvertMass(d,S(from),S(to))));
