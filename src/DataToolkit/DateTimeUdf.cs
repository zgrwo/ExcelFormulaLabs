...
namespace ExcelVbaLibraries.DataToolkit
{
    public static class DateTimeUdf
    {
        ...
        [ExcelFunction(Name="DT.WEEKDAY", Description="Day of week as number (Sunday=1, Saturday=7, matches VBA Weekday)")] public static object UDF_DT_WKD(object d)=>OutputWrapper.WrapError(()=>ElementWiseMapper.MapOver<double,long>(d,x=>DateTimeCore.Weekday(D(x))));
        ...
        [ExcelFunction(Name="DT.SOW", Description="Start of week date. sd = start day (0=Sun, 1=Mon, ...; default 1=Mon)")] public static object UDF_DT_SOW(object d,object sd)=>OutputWrapper.WrapError(()=>ElementWiseMapper.MapOver<double,double>(d,x=>DateTimeCore.StartOfWeek(D(x),(DayOfWeek)(int)(sd is ExcelDna.Integration.ExcelMissing?1L:InputNormalizer.ToLong(sd))).ToOADate()));
        [ExcelFunction(Name="DT.EOW", Description="End of week date. sd = start day (0=Sun, 1=Mon, ...; default 1=Mon)")] public static object UDF_DT_EOW(object d,object sd)=>OutputWrapper.WrapError(()=>ElementWiseMapper.MapOver<double,double>(d,x=>DateTimeCore.EndOfWeek(D(x),(DayOfWeek)(int)(sd is ExcelDna.Integration.ExcelMissing?1L:InputNormalizer.ToLong(sd))).ToOADate()));
        ...
        [ExcelFunction(Name="DT.WOM", Description="Week-of-month ordinal (1-5). sd = start day (0=Sun, ...; default 1=Mon)")] public static object UDF_DT_WOM(object d,object sd)=>OutputWrapper.WrapError(()=>(long)DateTimeCore.WeekOfMonth(D(d),(DayOfWeek)(int)(sd is ExcelDna.Integration.ExcelMissing?1L:InputNormalizer.ToLong(sd))));
