...
namespace ExcelFormulaLabs.DataToolkit
{
    ...
    internal static class StringCore
    {
        ...
        { if(string.IsNullOrEmpty(t))return""; char f=char.ToUpperInvariant(t[0]); var sb=new StringBuilder();sb.Append(f);char pc=Sdx(f);
          for(int i=1;i<t.Length&&sb.Length<4;i++){char uc=char.ToUpperInvariant(t[i]);char c=Sdx(uc); if(c!='0'&&c!=pc){sb.Append(c);pc=c;}else if(c=='0'&&uc is not('H'or'W'))pc='0';} while(sb.Length<4)sb.Append('0'); return sb.ToString(); }
        // NARA Soundex encoding: BFPV→1, CGJKQSXZ→2, DT→3, L→4, MN→5, R→6, vowels+HWY→0
