...
namespace ExcelVbaLibraries.DataToolkit
{
    public static class RegexUdf
    {
        ...
        [ExcelFunction(Name="REGEX.MATCH", Description="Nth regex match substring. n=1 (default/first), n=-1 (last), or empty string if no match")] public static object UDF_RX_MATCH(object i,object p,object ic,object n=null!)=>OutputWrapper.WrapError(()=>ElementWiseMapper.MapOverMulti<string,string,string>(i,p,(x,y)=>RegexCore.RegexMatch(x,y,InputNormalizer.ToLong(n),InputNormalizer.ToBool(ic))));
        ...
        [ExcelFunction(Name="REGEX.REPLACE", Description="Replace the nth regex match. n=0/omit=all (default), n=1=first, n=-1=last")] public static object UDF_RX_REPL(object i,object p,object r,object ic,object n=null!)=>OutputWrapper.WrapError(()=>ElementWiseMapper.MapOver<string,string>(i,s=>RegexCore.RegexReplace(s,InputNormalizer.ToString(p),InputNormalizer.ToString(r),InputNormalizer.ToLong(n),InputNormalizer.ToBool(ic))));
        [ExcelFunction(Name="REGEX.SPLIT", Description="Split string by regex delimiter. n=0/omit=all splits (default), n>0=max n splits")] public static object UDF_RX_SPLIT(object i,object p,object ic,object n=null!)=>OutputWrapper.WrapError(()=>RegexCore.RegexSplit(InputNormalizer.ToString(i),InputNormalizer.ToString(p),InputNormalizer.ToLong(n),InputNormalizer.ToBool(ic)));
        ...
        [ExcelFunction(Name="REGEX.ISMATCH", Description="Case-insensitive regex match test (alias for REGEX.TEST with ic=true)")] public static object UDF_RX_ISMATCH(object i,object p)=>OutputWrapper.WrapError(()=>ElementWiseMapper.MapOverMulti<string,string,bool>(i,p,(x,y)=>RegexCore.RegexTest(x,y,true)));
    }
}
