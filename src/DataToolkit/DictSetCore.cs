...
namespace ExcelVbaLibraries.DataToolkit
{
    internal static class DictSetCore
    {
        ...
        internal static object[] Intersect(object[] a, object[] b) { var sb=new HashSet<string>(b.Select(ComparisonUtils.SafeKey)); return a.Where(x=>sb.Contains(ComparisonUtils.SafeKey(x))).ToArray(); }
        internal static object[] Union(object[] a, object[] b) { var s=new HashSet<string>(); var r=new List<object>(); foreach(var x in a.Concat(b)){if(s.Add(ComparisonUtils.SafeKey(x)))r.Add(x);} return r.ToArray(); }
        internal static object[] Except(object[] a, object[] b) { var sb=new HashSet<string>(b.Select(ComparisonUtils.SafeKey)); return a.Where(x=>!sb.Contains(ComparisonUtils.SafeKey(x))).ToArray(); }
