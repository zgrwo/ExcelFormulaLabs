...
namespace ExcelFormulaLabs.DataToolkit
{
    ...
    internal static class DateTimeCore
    {
        ...
        // 契约（api-reference/user-manual）：返回当月第几周 1–6，含 1 号的那一周为第 1 周。
        // 旧实现按 start_day 在 [月初, d] 的出现次数计数 → 6/1(周六) 返回 0、6/15 返回 2，
        // 与文档示例（6/1→1、6/15→3）矛盾；Phase 4 交叉对照发现（2026-09-23）。
        // 上限 6 而非 5（review-2026-09-24 R1-05）：1 号为周日且月长 31 天（如 2026-03）
        // 时周一为周首会跨 6 个日历周（2/23 起算）；"1–5" 在数学上不可达。周日为周首时
        // 该月仍为 5（1 号即第 1 周起点）。
        ...
        internal static DateTime AddWorkdays(DateTime s, long days)
        {
            AssertValidDate(s);
            const long maxWorkdays = 100_000;
            // Math.Abs(long.MinValue) 抛 OverflowException（误导性异常，非业务校验）；
            // long.MinValue 远超上限，直接判越界。
        ...
        internal static long WorkdaysBetween(DateTime s, DateTime e) { AssertValidDate(s); AssertValidDate(e); const long maxSpan = 100_000; // 只判上界会在 e < s 时失效——TotalDays 为负，天然 < maxSpan → 守卫失效 → 静默返回 0。Excel NETWORKDAYS 契约：start > end 返回负工作日数（对称），故用 Math.Abs 判跨度，再按方向计算。
        ...
        internal static long DateDiff(string u, DateTime d1, DateTime d2) { AssertValidDate(d1); AssertValidDate(d2); return u.ToUpperInvariant() switch
        {
            ...
            "M" or "MONTH" => (d2.Year - d1.Year) * 12 + d2.Month - d1.Month - (d2.Day < d1.Day ? 1 : 0),
            "Y" or "YEAR" => d2.Year - d1.Year - (d2.Month < d1.Month || (d2.Month == d1.Month && d2.Day < d1.Day) ? 1 : 0),
            "W" or "WEEK" => (long)(d2 - d1).TotalDays / 7,
            _ => throw new ArgumentException(ErrorMsg.Get("DT_UnknownUnit", u))
        }; }

        private static void AssertValidDate(DateTime d)
        { if (d == DateTime.MinValue) throw new ArgumentException(ErrorMsg.Get("DT_InvalidDate")); }
    }
}
