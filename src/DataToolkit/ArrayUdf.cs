...
namespace ExcelVbaLibraries.DataToolkit
{
    public static class ArrayUdf
    {
        ...
        [ExcelFunction(Name="ARR.RANGE", Description="Generate numeric sequence from s to e by step")] public static object UDF_ARR_RANGE(object s,object e,object step)=>OutputWrapper.WrapError(()=>{double st=InputNormalizer.ToDouble(s),en=InputNormalizer.ToDouble(e),sp=InputNormalizer.ToDouble(step);if(sp<=0)sp=1;if(double.IsNaN(st)||double.IsNaN(en)||st>en)return System.Array.Empty<object>();int n=(int)System.Math.Round((en-st)/sp)+1;if(n<1)n=1;if(n>100000)n=100000;var r=new List<object>(n);for(int i=0;i<n;i++)r.Add(st+i*sp);return r.ToArray();});
