...
| 2-3 参数，广播 | `MapOverMulti<T1,T2,TOut>` | 标量广播到数组尺寸 | 尺寸不匹配→`ExcelError.Value`；任一输入展平为空→返回 `ExcelEmpty.Value`（渲染空白；顶层 `null` 会被 Excel 显示为 `#NUM!`，R2-6） |
...
// ③ MapOverMulti — 2-3 参数广播，尺寸不匹配 → ExcelError.Value；空输入 → ExcelEmpty.Value
=> OutputWrapper.WrapError(() =>
...
- MathNet 5.0.0 QR 不支持宽矩阵 m<n → 改为抛出 `NotSupportedException` 并引导用户使用 SVD 或转置（零填充方阵会产生错误结果，已废弃）
