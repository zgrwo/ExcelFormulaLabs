...
# 场景：7 个 fixture，验证零断言/恒真断言/存在性断言预算的检测能力，
...
Write-Host "[1/7] 干净树应通过（含表达式体/helper/注释内分号不误报）"
...
Write-Host "[2/7] 零断言应被检出 (ZERO_ASSERT)"
...
Write-Host "[3/7] 恒真断言应被检出 (TAUTOLOGY)"
...
Write-Host "[4/7] 存在性断言超预算应被检出 (PRESENCE_ONLY)"
...
# --- 场景 5：存在性断言在预算内 → 通过 ---
Write-Host "[5/7] 存在性断言在预算内应通过"
Run-Check $presence "" $false @("-MaxWarn", "1")

# --- 场景 6：注释掉的断言不算断言（防注释规避门禁）---
Write-Host "[6/7] 注释内 Should() 不得冒充断言 (ZERO_ASSERT)"
