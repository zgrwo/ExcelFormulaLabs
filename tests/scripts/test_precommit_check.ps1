...
# 场景：7 个 fixture，逐一验证 6 项检查的检测能力（含修复后的自校验检测）。
...
# --- 场景 2：裸 catch ---
Write-Host "[2/7] 裸 catch 应被检出 (BARE_CATCH)"
...
Write-Host "[3/7] 自校验应被检出 (SELF_CHECK)"
...
Write-Host "[4/7] IntelliSense 泄漏应被检出 (INTELLISENSE_LEAK)"
...
Write-Host "[7/7] object[,] 无 hasHeaders 应被检出 (HAS_HEADERS)"
