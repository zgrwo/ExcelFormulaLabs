...
| 范围 | Skill 文件 | 内容 |
| :--- | :--- | :--- |
| Core / Foundation / 基础逻辑 | `skills/excel-dna-project/skill.md` | 编码规范、架构、MapOver 变体、表头/哨兵契约、测试模式、全量测试命令 |
...
| 全量测试 | ① `bash scripts/verify-docs.sh` ② `dotnet test` ③ `dotnet test --filter "CrossVal\|PythonCrossValidation"` ④ `dotnet build -c Debug && dotnet build -c Release` |
