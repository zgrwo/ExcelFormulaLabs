...
**在 Excel 里直接用 `=STATS.MEAN()`、`=STR.REVERSE()`、`=JSON.PARSE()` 等 219 个函数。** 基于 C# 高性能实现，Python 级精度。自带 IntelliSense 自动补全。
...
输入 `=STATS.` 时 Excel 自动弹出全部统计函数和说明。219 个函数的完整说明见 [API 参考](docs/api-reference.md)。
...
| `LINALG.*` (19) | 矩阵行列式/求逆/特征值/SVD/QR/LU/Cholesky | `=LINALG.SOLVE(A1:C3, D1:D3)` |
...
`FS.*` 函数默认无路径限制（与 VBA 行为一致）。分发场景下可启用沙箱，将文件 I/O 限制在指定目录：

```csharp
// 在 AddIn 的 AutoOpen() 中设置（src/DataToolkit/AddIn.cs）
FileSystemCore.SandboxRoot = @"C:\Users\Public\Documents";
```

设置后，`FS.READ("C:\Windows\System32\config\SAM")` 等越界访问将返回 `#VALUE!`。

### SQL 注入防护

`SQL.QUERY` / `SQL.JOIN` 的数据 INSERT 使用参数化查询（`@p0, @p1, ...`），列名经字母数字消毒。用户提供的 SQL 语句本身**不可参数化**——请在可信输入上使用，或通过沙箱限制数据源。
...
- [API 参考](docs/api-reference.md) — 219 个函数的完整签名、参数说明、返回值解读
