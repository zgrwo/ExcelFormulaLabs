...
DOC_UDF=$(grep -cE '^\| `[A-Z]+\.[A-Z]' rules/api-reference.md 2>/dev/null || echo 0)
...
while IFS= read -r udf; do
    [ -z "$udf" ] && continue
    grep -q "$udf" rules/api-reference.md || { check "UDF全覆盖: $udf" "缺失"; }
done < <(grep -roh 'ExcelFunction(Name *= *"[^"]*"' src/ --include="*.cs" 2>/dev/null | sed 's/ExcelFunction(Name *= *"//;s/"//' | sort -u)
check "UDF全覆盖检查完成" "OK"

# 3. skill.md包含RangeExport
grep -q 'RangeExport' skills/excel-dna-project.md && check "skill.md RangeExport" "OK" || check "skill.md RangeExport" "缺失"

# 4. 架构术语现代性
grep -q 'MapOver' skills/excel-dna-project.md && check "skill.md MapOver术语" "OK" || check "skill.md MapOver术语" "缺失"
...
DOC_VER=$(grep -oP 'MathNet\.Numerics\s+\K[0-9.]+' rules/context.md 2>/dev/null || echo "?")
