...
> 版本：v2.4.1 <!-- x-release-please-version -->
...
> 给定输出目标反推可调参数（工艺调机）。列角色由表头前缀识别：`Incoming*`/`来料*`（已知条件）、`Variable*`/`可调*`/`变量*`（可调参数）、`Fixed*`/`固定*`（固定参数，留空取历史中位数）、`Output*`/`输出*`（输出/目标）、`SharedOutput*`/`共享输出*`（输出，且同组共享一条速率 g，见 rate）。前缀不区分大小写，未识别列忽略。
> 请求行 = 任意可调列留空、至少一个输出目标有值（可内嵌 data，或用独立 request 表）。模型 `auto`/`linear`/`poly`（二次含两两交互）/`rate`（线性速率 g）/`rate_poly`（二次速率 g，ADR-0009），`auto` 按交叉验证 R² 选优（差值 <1e-9 时优先 linear，其次 poly，最后 rate；普通输出的 auto 不含 rate_poly）。任一候选的交叉验证失败（如设计矩阵精确共线/秩亏）时按"跳过"处理，全部候选不可用才返回 #VALUE!。确定性：同 seed 可复现（`XorShift64`，双 TFM 一致）。规模上限（超限返回 #VALUE!，四个函数统一生效）：历史 5000 行 / 请求 200 行 / 特征列 50 / 可调 20 / 输出 20 / poly 展开 100 项 / max_starts 50。样本下限：`auto` 选型与 `SOLVE.QUALITY` 依赖交叉验证，历史行须 ≥5，否则 `#VALUE!`（显式模型只需 行数 ≥ 展开项数+1）。
>
