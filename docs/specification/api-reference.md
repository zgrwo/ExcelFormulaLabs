...
> 请求行 = 任意可调列留空、至少一个输出目标有值（可内嵌 data，或用独立 request 表）。模型 `auto`/`linear`/`poly`（二次含两两交互）/`rate`（线性速率 g）/`rate_poly`（二次速率 g，ADR-0009），`auto` 按交叉验证 R² 选优（差值 <1e-9 时优先 linear，其次 poly，最后 rate；普通输出的 auto 不含 rate_poly）。确定性：同 seed 可复现（`XorShift64`，双 TFM 一致）。规模上限（超限返回 #VALUE!，四个函数统一生效）：历史 5000 行 / 请求 200 行 / 特征列 50 / 可调 20 / 输出 20 / poly 展开 100 项 / max_starts 50。样本下限：`auto` 选型与 `SOLVE.QUALITY` 依赖交叉验证，历史行须 ≥5，否则 `#VALUE!`（显式模型只需 行数 ≥ 展开项数+1）。
>
