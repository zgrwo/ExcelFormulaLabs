...
**返回**：`string` — 文件全部内容。UTF-8 编码，自动识别并剥离 BOM。
...
**返回**：`bool`。覆盖写入。编码为 UTF-8 **含 BOM**（.NET `Encoding.UTF8` 语义）；
`FS.READ` 读回时自动剥离，二者往返内容一致。如需无 BOM 输出，写入后用其他工具转换。
