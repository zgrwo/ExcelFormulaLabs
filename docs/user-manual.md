...
→ 0.2158...  (p > 0.05，均值 28.8 与 25 无显著差异)
```
...
`=STATS.PERCENTILE(array, k)` 中 k 为 0~100。使用 R7 插值算法（对标 Excel `PERCENTILE.EXC`）。IQR 内部用 `k=25` 和 `k=75` 计算。
...
> 条件数大 → 矩阵接近奇异 → 求解不可靠。此时可考虑 Ridge 回归（共线性场景）或数据标准化。
...
| LU | `LU_L` + `LU_U` + `LU_P` | PA = LU | 行列式计算、线性方程组 |
